...
declare namespace log="http://cefn.com/readinglog/log" ;

declare variable $filepaths as xs:string external;

declare variable $collection := for $token in tokenize($filepaths, ',') return doc(resolve-uri($token));
...
log:webpage(
    for $doc in $collection return $doc//h1
)